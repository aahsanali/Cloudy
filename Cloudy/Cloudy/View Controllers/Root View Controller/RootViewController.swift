    ...
    private func fetchWeatherData() {
        ...
        dataManager.weatherDataForLocation(latitude: latitude, longitude: longitude) { [weak self] (result) in
            ...
            case .success(let weatherData):
                ...
                self?.dayViewController.viewModel = DayViewViewModel(weatherData: weatherData)

                // Configure Week View Controller
                self?.weekViewController.viewModel = WeekViewViewModel(weatherData: weatherData.dailyData) 
            case .failure:
                // Notify User
